...
void init(data *d) {
  ...
  init_hit(d);
  d->a.quiet = false; 
  // Create output directory unless text-only.
  if (! d->a.text_only) {
    if (create_output_directory(d->a.out_dir, d->a.clobber, !d->a.quiet)) {
      die("Unable to create output directory %s.\n", d->a.out_dir);
    }
    // Open text output file
    d->txt_filename = make_path_to_file(d->a.out_dir, "glam2scan.txt");
    d->html_filename = make_path_to_file(d->a.out_dir, "glam2scan.html");
